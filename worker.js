...
export default {
    async fetch(request, env, ctx) {
        ...
        if (isRoot && contentType && contentType.includes("text/html")) {
            ...
            const logoJs = `document.addEventListener("DOMContentLoaded",function(){let e=document.createElement("div");e.style.cssText="z-index:9999!important;width:100vw;height:30px;background-color:yellow;position:absolute;";let t=document.createElement("span");t.style.cssText="left:50%;color:black;position:absolute;top:50%;transform:translate(-50%,-50%);width:100vw;text-align:center;",t.innerHTML="<b>WARNING!</b> We will soon be wiping all files due to multiple phishing reports by AdGuard, Fortinet and others. <i><b>Download your files before 2024-11-15.</b></i>",e.appendChild(t),document.body.insertBefore(e,document.body.firstChild)});const style=document.documentElement.style;function getLogoSrc(){let e=style.getPropertyValue("color-scheme").trim();return"dark"===e?"https://clickette.net/assets-homepage/img/wordmark-white.svg":"https://clickette.net/assets-homepage/img/wordmark-black.svg"}function setTargetInnerHTML(){let e=document.querySelector("#__next > div > header > div > h1");if(e&&"Clickette"===e.innerHTML.trim()){let t=getLogoSrc();return e.innerHTML=\`<img id="logo" src="\${t}" height="30" style="margin-top:-8px;">\`,e}return null}function updateLogo(){let e=document.getElementById("logo");e&&(e.src=getLogoSrc())}const observer=new MutationObserver(e=>{let t=document.querySelector("#__next > div > header > div > h1");t&&"Clickette"===t.innerHTML.trim()&&setTargetInnerHTML()});let targetElement=setTargetInnerHTML();const config={childList:!0,subtree:!0};observer.observe(document,config);const targetObserver=new MutationObserver(e=>{for(let t of e)"childList"===t.type&&(targetElement=setTargetInnerHTML(),targetObserver.observe(targetElement,{childList:!0,subtree:!0}))});targetObserver.observe(document,{childList:!0,subtree:!0});const themeObserver=new MutationObserver(e=>{for(let t of e)"style"===t.attributeName&&updateLogo()});themeObserver.observe(document.documentElement,{attributes:!0,attributeFilter:["style"]}),updateLogo();`;
